...
import { Container, Stack, Title, Text, Paper, Divider, Badge, Group, ThemeIcon, Code, Image, Tooltip, CopyButton, Table, Anchor, NumberInput, Checkbox, SimpleGrid, SegmentedControl, UnstyledButton, Collapse } from '@mantine/core';
import { BookOpen, Egg, Heart, Info, MapPin, Sliders, TrendingUp, Copy, Star, ChevronDown, ChevronUp } from 'lucide-react';
import { AreaChart, Area, XAxis, YAxis, CartesianGrid, ResponsiveContainer, Tooltip as RTooltip } from 'recharts';
import { MOUNT_XP_CUMULATIVE, MOUNT_XP_PER_LEVEL } from '@/data/mountXp';
...
const XP_PER_LEVEL_DATA = Array.from({ length: 199 }, (_, i) => ({
  niveau: i + 2,
  xp: MOUNT_XP_PER_LEVEL[i],
}));

const XP_CUMUL_DATA = Array.from({ length: 200 }, (_, i) => ({
  niveau: i + 1,
  cumul: MOUNT_XP_CUMULATIVE[i],
}));

function XpSection() {
  const [view, setView] = useState<'per-level' | 'cumulative'>('per-level');
  const [showTables, setShowTables] = useState(false);
  const [openGroup, setOpenGroup] = useState<number | null>(null);

  const chartData = view === 'per-level' ? XP_PER_LEVEL_DATA : XP_CUMUL_DATA;

  const groups = Array.from({ length: 20 }, (_, g) => {
    const startLevel = g * 10 + 1;
    const endLevel = Math.min(startLevel + 9, 200);
    const totalXp = Array.from({ length: endLevel - startLevel + 1 }, (_, j) => {
      const lvl = startLevel + j;
      return lvl < 200 ? MOUNT_XP_PER_LEVEL[lvl - 1] : 0;
    }).reduce((a, b) => a + b, 0);
    return { g, startLevel, endLevel, totalXp };
  });
  ...
  return (
    <Stack gap="md">
      <SegmentedControl
        size="sm"
        value={view}
        onChange={(v) => setView(v as 'per-level' | 'cumulative')}
        data={[
          { value: 'per-level', label: 'XP par niveau' },
          { value: 'cumulative', label: 'XP cumulée' },
        ]}
      />
      <Paper withBorder p="sm" radius="md" bg="gray.0">
        <ResponsiveContainer width="100%" height={220}>
          <AreaChart data={chartData}>
            ...
            <XAxis dataKey="niveau" interval={19} tick={{ fontSize: 12 }} />
            <YAxis width={52} tickFormatter={formatXp} tick={{ fontSize: 12 }} />
            <RTooltip
              content={({ active, payload }) => {
                if (!active || !payload?.length) return null;
                const d = payload[0].payload;
                const lvl = d.niveau as number;
                if (view === 'per-level') {
                  return (
                    <Paper withBorder p="xs" radius="sm" shadow="sm" bg="white">
                      <Text size="xs" fw={700} c="dark">Niveau {lvl - 1} → {lvl}</Text>
                      <Text size="xs" c="dimmed">XP requise : <Text component="span" fw={600} c="green.7">{(d.xp as number).toLocaleString('fr-FR')}</Text></Text>
                      <Text size="xs" c="dimmed">XP cumulée au niv. {lvl} : <Text component="span" fw={600} c="dark">{MOUNT_XP_CUMULATIVE[lvl - 1].toLocaleString('fr-FR')}</Text></Text>
                    </Paper>
                  );
                }
                return (
                  <Paper withBorder p="xs" radius="sm" shadow="sm" bg="white">
                    <Text size="xs" fw={700} c="dark">Niveau {lvl}</Text>
                    <Text size="xs" c="dimmed">XP cumulée : <Text component="span" fw={600} c="teal.7">{(d.cumul as number).toLocaleString('fr-FR')}</Text></Text>
                    {lvl > 1 && <Text size="xs" c="dimmed">XP pour ce niveau : <Text component="span" fw={600} c="dark">{MOUNT_XP_PER_LEVEL[lvl - 2].toLocaleString('fr-FR')}</Text></Text>}
                  </Paper>
                );
              }}
            />
            <Area
              type="monotone"
              dataKey={view === 'per-level' ? 'xp' : 'cumul'}
              stroke={view === 'per-level' ? '#40c057' : '#12b886'}
              fill={view === 'per-level' ? '#40c057' : '#12b886'}
              fillOpacity={0.2}
              strokeWidth={2}
              dot={false}
      ...
      <Paper withBorder radius="md" style={{ overflow: 'hidden' }}>
        <UnstyledButton w="100%" px="sm" py="xs" onClick={() => setShowTables(!showTables)}>
          <Group justify="space-between" wrap="nowrap">
            <Text size="sm" fw={700} c="dark">Tableaux d'XP détaillés</Text>
        ...
        <Collapse in={showTables}>
          ...
          <Stack gap="xs" p="xs">
            {groups.map(({ g, startLevel, endLevel, totalXp }) => (
              <Paper key={g} withBorder radius="sm" style={{ overflow: 'hidden' }}>
                <UnstyledButton w="100%" px="sm" py="xs" onClick={() => setOpenGroup(openGroup === g ? null : g)}>
                  <Group justify="space-between" wrap="nowrap">
                    <Text size="sm" fw={600} c="dark">Niveaux {startLevel} – {endLevel}</Text>
                    <Group gap="xs" wrap="nowrap">
                      <Badge color="green" variant="light" size="sm">{totalXp.toLocaleString('fr-FR')} XP</Badge>
                      {openGroup === g ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                    </Group>
                  </Group>
                </UnstyledButton>
                <Collapse in={openGroup === g}>
                  <Divider color="gray.2" />
                  <Table fz="xs" withColumnBorders>
                    <Table.Thead bg="gray.0">
                      <Table.Tr>
                        <Table.Th>Niveau</Table.Th>
                        <Table.Th>XP cumulée</Table.Th>
                        <Table.Th>XP pour ce niveau</Table.Th>
                      </Table.Tr>
                    </Table.Thead>
                    <Table.Tbody>
                      {Array.from({ length: endLevel - startLevel + 1 }, (_, j) => {
                        const level = startLevel + j;
                        return (
                          <Table.Tr key={level}>
                            <Table.Td fw={600}>{level}</Table.Td>
                            <Table.Td>{MOUNT_XP_CUMULATIVE[level - 1].toLocaleString('fr-FR')}</Table.Td>
                            <Table.Td>{level < 200 ? MOUNT_XP_PER_LEVEL[level - 1].toLocaleString('fr-FR') : '—'}</Table.Td>
                          </Table.Tr>
                        );
                      })}
                    </Table.Tbody>
...
function BreedingSimulator() {
  const [level1, setLevel1] = useState(100);
  const [level2, setLevel2] = useState(100);
  const [optimakina, setOptimakina] = useState(false);

  const bonusLevels = (level1 + level2) * 0.15;
  const bonusOptimakina = optimakina ? 10 : 0;
  const prob = Math.min(100, 30 + bonusLevels + bonusOptimakina);
  const isMax = prob >= 100;

  const probColor = isMax ? 'green.7' : prob >= 70 ? 'teal.7' : prob >= 50 ? 'orange.7' : 'red.6';

  return (
    <Paper withBorder p="md" radius="md" bg="teal.0" style={{ borderColor: 'var(--mantine-color-teal-3)' }}>
      <Text fw={700} size="sm" c="teal.8" mb="sm">Simulateur</Text>
      <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="sm">
        <Stack gap={4}>
          <Text size="sm" fw={600} c="dark">Niveau Parent 1</Text>
          <NumberInput
            value={level1}
            onChange={(v) => setLevel1(typeof v === 'number' ? Math.max(1, Math.min(200, v)) : 1)}
            min={1} max={200}
          />
        </Stack>
        <Stack gap={4}>
          <Text size="sm" fw={600} c="dark">Niveau Parent 2</Text>
          <NumberInput
            value={level2}
            onChange={(v) => setLevel2(typeof v === 'number' ? Math.max(1, Math.min(200, v)) : 1)}
            min={1} max={200}
          />
        </Stack>
      </SimpleGrid>
      <Checkbox
        ...
        label="Optimakina (+10 %)"
        checked={optimakina}
        onChange={(e) => setOptimakina(e.currentTarget.checked)}
      />
      <Divider my="sm" color="teal.2" />
      <Group justify="space-between" align="flex-end" wrap="wrap" gap="xs">
        <Stack gap={2}>
          <Group gap="xs">
            <Text size="xs" c="dimmed">Base</Text>
            <Text size="xs" fw={600} c="dark">30 %</Text>
            <Text size="xs" c="dimmed">+ niveaux</Text>
            <Text size="xs" fw={600} c="teal.7">+{bonusLevels.toFixed(2)} %</Text>
            {optimakina && (
              <>
                <Text size="xs" c="dimmed">+ Optimakina</Text>
                <Text size="xs" fw={600} c="yellow.7">+10 %</Text>
              </>
            )}
          </Group>
          <Group gap="xs" align="baseline">
            <Text size="xs" c="dimmed" tt="uppercase" lts={0.5}>Génération suivante</Text>
            <Text fw={800} size="xl" c={probColor}>{prob.toFixed(2)} %</Text>
          </Group>
        </Stack>
        {isMax && <Badge color="green" variant="filled" size="lg">Garanti ✓</Badge>}
      </Group>
    </Paper>
  );
}
